...
import org.grapheco.pandadb.PandaQueryExecutionEngine;
...
//    private RecoveryCleanupWorkCollector recoveryCleanupWorkCollector;//TODO enale recovery
...
//        this.transactionsMemoryPool = context.getTransactionsMemoryPool(); // rocksdb has memory pool for tx
...
//            var providerSpi = QueryEngineProvider.spi( internalLogProvider, databaseMonitors, scheduler, life, getKernel(), databaseConfig );
            this.executionEngine = PandaQueryExecutionEngine.instance; //QueryEngineProvider.initialize( databaseDependencies, databaseFacade, engineProvider, isSystem(), providerSpi );
...
//    }
    ...
    public synchronized void stop()
    {
        ...
        life.stop();
//        awaitAllClosingTransactions();//TODO use neo kernel tx
        life.shutdown();
        started = false;
        initialized = false;
    }
...
//    }
    ...
    public synchronized void upgrade( boolean startAfterUpgrade ) throws IOException{
        throw new UnsupportedOperationException("upgrade db");
    }
