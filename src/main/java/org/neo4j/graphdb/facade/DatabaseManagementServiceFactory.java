...
//TODO 修改它，使其不依赖AbstractEditionModule，尽量不依赖databaseManager。
...
public class DatabaseManagementServiceFactory
{
    ...
    public DatabaseManagementServiceFactory( DbmsInfo dbmsInfo,  Function<GlobalModule, AbstractEditionModule> no_use )
    {
...
//        AbstractEditionModule edition = editionFactory.apply( globalModule );
        ...
        DatabaseManager<?> databaseManager = createDatabaseManager( globalModule );
...
//        dumpDbmsInfo( logService.getUserLog( getClass() ), databaseManager.getDatabaseContext( NAMED_SYSTEM_DATABASE_ID ).get().databaseFacade() );
    ...
    {
        var databaseManager = new DefaultDatabaseManager( globalModule );
        var databaseStateService = new CommunityDatabaseStateService( databaseManager );
...
//        var globalReadOnlyChecker = createGlobalReadOnlyChecker( databaseManager, globalModule.getGlobalConfig(),
...
//        var databaseReferenceRepo = databaseReferenceRepo;

//        var fabricServicesBootstrap = new FabricServicesBootstrap.Community( globalModule.getGlobalLife(),
